...
function setLocal(k: string, v: string | null) {
  ...
  if (v === null) (window as any).localStorage.removeItem(k); else (window as any).localStorage.setItem(k, v);
}
...
describe('apiClient config', () => {
  beforeAll(() => {
    (globalThis as any).XMLHttpRequest = class {
      ...
      status = 200;
      statusText = 'OK';
      response: any = new Blob([JSON.stringify({ ok: true })], { type: 'application/json' });
      timeout = 0;
  ...
  it('attaches X-API-Key for admin endpoints', async () => {
    ...
    (window as any).location = { origin: 'http://localhost:3000', host: 'localhost:3000', protocol: 'http:' } as any;
    const res = await apiFetch('/api/admin/test', { method: 'GET', admin: true });
