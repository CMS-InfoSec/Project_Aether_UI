...
export default function AdminDashboard() {
  ...
  const loadDailyReport = async () => {
    ...
    try {
      ...
      toast({ title: 'Daily Report Error', description: 'Failed to load daily report data', variant: 'destructive' });
    } finally {
  ...
  const loadWeeklyReport = async () => {
    ...
    try {
      ...
      toast({ title: 'Weekly Report Error', description: 'Failed to load weekly report data', variant: 'destructive' });
    } finally {
  ...
  const loadPerAssetReport = async () => {
    ...
    try {
      ...
      toast({ title: 'Per-Asset Report Error', description: 'Failed to load per-asset report data', variant: 'destructive' });
    } finally {
  ...
  const exportCSV = async (reportType: 'daily' | 'weekly' | 'per-asset') => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: 'Export Successful', description: `${reportType} report exported to CSV successfully.` });
      } else {
  ...
  const downloadBacktestReport = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        document.body.removeChild(a);
        window.URL.revokeObjectURL(url);
        toast({ title: 'Download Complete', description: 'Backtest report downloaded successfully.' });
      } else {
        const errorData = await response.json();
        if (response.status === 404) {
          toast({ title: 'No Report Available', description: 'No backtest report found. Please run a backtest first.', variant: 'destructive' });
        } else {
      ...
      toast({ title: 'Download Failed', description: 'Failed to download backtest report. Please try again.', variant: 'destructive' });
    } finally {
    ...
    try {
      ...
      if (response.ok) {
        setNotificationData(prev => {
          if (!prev) return prev;
          return { ...prev, notifications: prev.notifications.map(n => n.id === id ? { ...n, read } : n) };
        });
      }
    } catch (error) {
  ...
  const getSeverityBadgeVariant = (severity: string) => ({ error: 'destructive', warning: 'secondary', success: 'default', info: 'outline' }[severity as keyof any] || 'outline');
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Controls, governance, reports, and operational tooling</p>
        </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="flex items-center justify-between space-x-4">
                ...
                <Button variant="outline" onClick={refreshAllData} disabled={Object.values(isRefreshing).some(Boolean)}>
            ...
            <TabsList className="grid w-full grid-cols-4">
              ...
              <TabsTrigger value="notifications" className="relative">
                ...
                {notificationData?.summary.unread > 0 && (
                  <Badge className="ml-2 h-5 w-5 rounded-full p-0 text-xs">{notificationData.summary.unread}</Badge>
                )}
              </TabsTrigger>
            </TabsList>
              ...
              <WidgetErrorBoundary widgetName="Key Metrics">
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-accent">{dailyReport ? formatCurrency(dailyReport.totalReturn) : '--'}</div>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{dailyReport ? dailyReport.activePortfolios : '--'}</div>
                      <p className="text-xs text-muted-foreground">System managed</p>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold text-accent">{dailyReport ? formatPercentage(dailyReport.avgPerformance) : '--'}</div>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="text-2xl font-bold">{dailyReport ? dailyReport.topPerformer.asset : '--'}</div>
                      <p className="text-xs text-accent">{dailyReport ? formatPercentage(dailyReport.topPerformer.performance) : '--'} today</p>
                    </CardContent>
                  </Card>
                </div>
              </WidgetErrorBoundary>
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" onClick={loadDailyReport} disabled={isRefreshing.daily}>
                          {isRefreshing.daily ? <RefreshCw className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
                        </Button>
                      </div>
                    </CardHeader>
                    <CardContent>
                      {dailyReport ? (
                        <ResponsiveContainer width="100%" height={300}>
                          <LineChart data={dailyReport.dailyReturnsData}>
                            ...
                            <Tooltip />
                            <Legend />
                            <Line type="monotone" dataKey="returns" stroke="hsl(var(--primary))" strokeWidth={2} name="Returns (%)" />
                    ...
                    <CardHeader>
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="outline" size="sm" onClick={loadWeeklyReport} disabled={isRefreshing.weekly}>
                          {isRefreshing.weekly ? <RefreshCw className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
                        </Button>
                      </div>
                    </CardHeader>
                    <CardContent>
            ...
            <TabsContent value="reporting" className="space-y-6">
                        ...
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="text-lg font-semibold text-accent">{formatCurrency(dailyReport.totalReturn)}</p>
                            </div>
                            <div>
                              <p className="text-sm font-medium text-muted-foreground">Performance</p>
                              <p className="text-lg font-semibold">{formatPercentage(dailyReport.avgPerformance)}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <p className="text-lg font-semibold text-destructive">{formatPercentage(dailyReport.riskMetrics.maxDrawdown)}</p>
                            </div>
                          </div>
                          ...
                          <p className="text-xs text-muted-foreground">Last updated: {new Date(dailyReport.lastUpdated).toLocaleTimeString()}</p>
                        </div>
                      ) : (
                        ...
                        <div className="space-y-4">
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <p className="text-lg font-semibold text-accent">{formatCurrency(weeklyReport.weeklyReturn)}</p>
                            </div>
                            <div>
                              <p className="text-sm font-medium text-muted-foreground">Win Rate</p>
                              <p className="text-lg font-semibold">{formatPercentage(weeklyReport.winRate * 100)}</p>
                            </div>
                            <div>
                          ...
                          <div className="pt-2 border-t space-y-1">
                            <div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">Information Ratio:</span><span>{weeklyReport.performanceMetrics.informationRatio.toFixed(2)}</span></div>
                            <div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">Calmar Ratio:</span><span>{weeklyReport.performanceMetrics.calmarRatio.toFixed(2)}</span></div>
                            <div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">Sortino Ratio:</span><span>{weeklyReport.performanceMetrics.sortinoRatio.toFixed(2)}</span></div>
                          </div>
                          <p className="text-xs text-muted-foreground">Last updated: {new Date(weeklyReport.lastUpdated).toLocaleTimeString()}</p>
                        </div>
                      ) : (
                        <div className="flex items-center justify-center h-48">
                          <RefreshCw className="h-6 w-6 animate-spin text-muted-foreground" />
              ...
              <WidgetErrorBoundary widgetName="Export Controls">
                <Card>
                  ...
                  <CardContent>
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                      <Button variant="outline" onClick={() => exportCSV('daily')} disabled={isRefreshing.csv} className="w-full">
                        {isRefreshing.csv ? <RefreshCw className="h-4 w-4 animate-spin mr-2" /> : <Download className="h-4 w-4 mr-2" />}
                        Daily CSV
                      </Button>
                      <Button variant="outline" onClick={() => exportCSV('weekly')} disabled={isRefreshing.csv} className="w-full">
                        {isRefreshing.csv ? <RefreshCw className="h-4 w-4 animate-spin mr-2" /> : <Download className="h-4 w-4 mr-2" />}
                        Weekly CSV
                      </Button>
                      <Button variant="outline" onClick={() => exportCSV('per-asset')} disabled={isRefreshing.csv} className="w-full">
                        {isRefreshing.csv ? <RefreshCw className="h-4 w-4 animate-spin mr-2" /> : <Download className="h-4 w-4 mr-2" />}
                        Per-Asset CSV
                      </Button>
                      <Button onClick={downloadBacktestReport} disabled={isRefreshing.backtest} className="w-full">
                        {isRefreshing.backtest ? <RefreshCw className="h-4 w-4 animate-spin mr-2" /> : <FileText className="h-4 w-4 mr-2" />}
                        Backtest Report
                      </Button>
                    </div>
                    {isRefreshing.backtest && (
                      <div className="mt-4 space-y-2">
                        <div className="flex items-center justify-between text-sm"><span>Preparing download...</span><span>{backtestProgress}%</span></div>
            ...
            <TabsContent value="per-asset" className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Button variant="outline" onClick={loadPerAssetReport} disabled={isRefreshing.perAsset}>
                  {isRefreshing.perAsset ? <RefreshCw className="h-4 w-4 animate-spin mr-2" /> : <RefreshCw className="h-4 w-4 mr-2" />}
                  Refresh
                </Button>
              </div>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                      ...
                      <Card>
                        ...
                        <CardContent>
                          ...
                          <p className="text-xs text-accent">{formatPercentage(perAssetReport.summary.topPerformer.return)}</p>
                        </CardContent>
                      </Card>
                      <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        ...
                        <CardContent>
                          ...
                          <p className="text-xs text-destructive">{formatPercentage(perAssetReport.summary.bottomPerformer.return)}</p>
                        </CardContent>
                      </Card>
                      <Card>
                        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                            ...
                            <tbody>
                              {perAssetReport.assets.map((asset) => (
                                <tr key={asset.symbol} className="border-b hover:bg-muted/50">
                                  ...
                                  <td className={`text-right p-2 ${asset.dailyChange >= 0 ? 'text-accent' : 'text-destructive'}`}>{formatPercentage(asset.dailyChange)}</td>
                                  <td className={`text-right p-2 ${asset.weeklyChange >= 0 ? 'text-accent' : 'text-destructive'}`}>{formatPercentage(asset.weeklyChange)}</td>
                                  <td className={`text-right p-2 ${asset.monthlyChange >= 0 ? 'text-accent' : 'text-destructive'}`}>{formatPercentage(asset.monthlyChange)}</td>
                ...
                <Card>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <Button variant="outline" size="sm" onClick={markAllAsRead} disabled={!notificationData?.summary.unread}>Mark All Read</Button>
                        <Button variant="outline" size="sm" onClick={loadNotifications} disabled={isRefreshing.notifications}>
                          {isRefreshing.notifications ? <RefreshCw className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
                        </Button>
                      </div>
                    </div>
                  </CardHeader>
                    ...
                    {notificationData && (
                      <div className="grid grid-cols-4 gap-4 mb-6">
                        ...
                        <div className="text-center p-3 bg-primary/10 rounded-lg">
                          <div className="text-lg font-bold text-primary">{notificationData.summary.severityCounts.warning || 0}</div>
                    ...
                    <ScrollArea className="h-[500px]">
                      <div className="space-y-4">
                        {notificationData?.notifications.map((notification) => (
                          <div key={notification.id} className={`p-4 rounded-lg border transition-colors ${notification.read ? 'bg-muted/30 border-border' : 'bg-card border-primary/20 shadow-sm'}`}>
                            <div className="flex items-start space-x-3">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between">
                                  ...
                                  <div className="flex items-center space-x-2">
                                    <Badge variant={getSeverityBadgeVariant(notification.severity)} className="text-xs">{notification.severity}</Badge>
                                    {notification.actionRequired && (<Badge variant="outline" className="text-xs bg-primary/10">Action Required</Badge>)}
                                    {!notification.read && (<div className="w-2 h-2 bg-primary rounded-full"></div>)}
                                  </div>
                                </div>
                                <p className={`text-sm text-muted-foreground mt-1 ${expandedNotifications.has(notification.id) ? '' : 'line-clamp-2'}`}>{notification.message}</p>
                                ...
                                <div className="flex items-center justify-between mt-3">
                                  ...
                                  <div className="flex items-center space-x-2">
                                    {notification.metadata && (
                                      <Button variant="ghost" size="sm" onClick={() => toggleNotificationExpansion(notification.id)}>
                                        {expandedNotifications.has(notification.id) ? <EyeOff className="h-3 w-3" /> : <Eye className="h-3 w-3" />}
                                      </Button>
                                    )}
                                    <Button variant="ghost" size="sm" onClick={() => markNotificationAsRead(notification.id, !notification.read)}>
