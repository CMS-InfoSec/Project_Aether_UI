...
export default function ManipulationDefenseConsole() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        const data: any[] = Array.isArray(j?.data)
          ? j.data
          : Array.isArray(j)
            ? j
        ...
        const s = j?.settings || {};
        if (typeof s.operatorOverride === "boolean")
          setOverride(s.operatorOverride);
        if (typeof s.autoDefenseEnabled === "boolean")
          setAutoDefense(s.autoDefenseEnabled);
      } else {
        setEvents([]);
      }
    } catch {
  ...
  const toggleOverride = async (val: boolean) => {
    setOverride(val);
    try {
      await apiFetch("/api/defense/override", {
        method: "PATCH",
    ...
    <Card>
      <CardHeader className="flex items-start justify-between">
        <div>
          ...
          <CardDescription>
            Detected spoofing/shilling events, responses, and operator overrides
          </CardDescription>
        </div>
        ...
        {severeActive.slice(0, 3).map((e) => (
          ...
          >
            ...
            <div className="text-[11px] text-muted-foreground">
              Detected {new Date(e.detected_at).toLocaleTimeString()}
            </div>
          </div>
        ))}
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <label htmlFor="override" className="text-sm">
              Operator override (pause auto-defense)
            </label>
          </div>
          <Badge variant={override ? "destructive" : "outline"}>
            {override ? "OVERRIDE ACTIVE" : "AUTO-DEFENSE"}
          </Badge>
        </div>
        ...
        <ScrollArea className="h-64">
          <div className="space-y-2">
            {events.map((e) => (
              <div
                key={e.id}
                className={`p-2 border rounded-md ${e.status === "active" ? "border-red-200 bg-red-50/40" : ""}`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={
                        e.status === "active" ? "destructive" : "outline"
                      }
                ...
                <div className="mt-1 text-sm">
                  Actor: <span className="capitalize">{e.actor}</span> •
                  Response: <span>{e.response}</span>
                </div>
                {e.details && (
