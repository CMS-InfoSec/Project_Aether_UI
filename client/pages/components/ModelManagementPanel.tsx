...
export default function ModelManagementPanel() {
  ...
  const [modelType, setModelType] = useState<"forecast" | "rl_agent">(
    "rl_agent",
  );
    ...
    const load = async () => {
      try {
        ...
        const policies = Array.from(
          new Set(
            models.map((m) =>
              m.type === "rl" || m.type === "rl_agent" ? "PPO" : "LSTM",
            ),
          ),
        );
        ...
        setRegistry({
          models,
          policies: policies.length ? policies : ["PPO", "LSTM"],
          lastCheckpoint,
          supabase_degraded: Boolean(
            j?.supabase_degraded || j?.metadata?.supabase_degraded,
          ),
        });
        setDegraded(
          Boolean(j?.supabase_degraded || j?.metadata?.supabase_degraded),
        );
  ...
  const lastCheckpoint = useMemo(() => {
    ...
    try {
      return new Date(registry.lastCheckpoint).toLocaleString();
    } catch {
      return registry.lastCheckpoint;
    }
  }, [registry]);
  ...
  const runRetrain = async () => {
    ...
    try {
      const payload: any = {
        model_type: modelType === "rl_agent" ? "rl" : "forecast",
        coin: coinList,
      };
      if (modelType === "rl_agent")
        payload.algorithm = (policy || "PPO")
          .toLowerCase()
          .replace(/\s+/g, "_");
      else
        payload.architecture =
          policy.toLowerCase() === "transformer" ? "transformer" : "lstm";
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        {degraded && (
          <div className="p-3 border rounded bg-yellow-50 text-yellow-800 text-sm">
            Degraded: history storage unavailable; showing partial data.
          </div>
        )}
                  ...
                  {(registry?.models || []).slice(0, 10).map((m: any) => (
                    <tr key={m.modelId} className="border-b">
                      ...
                      <td className="p-2">
                        <Badge
                          variant={
                            m.status === "deployed" ? "outline" : "secondary"
                          }
                        >
                          {m.status}
                        </Badge>
