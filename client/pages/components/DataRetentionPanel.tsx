      ...
      <CardContent className="space-y-4">
        <div>
          ...
          <div className="flex items-center gap-3">
            <Slider value={[days]} min={7} max={3650} step={1} onValueChange={(v)=> setDays(v[0])} className="max-w-xl" disabled />
            <Button disabled>
              Save
            </Button>
          </div>
          <div className="text-xs text-muted-foreground">Range: 7–3650 days. Read-only: no writable backend endpoint.</div>
        </div>

        <div className="flex items-center gap-2">
          <Button variant="destructive" disabled>
            <Trash2 className="h-4 w-4 mr-2"/>Manual Purge
          </Button>
