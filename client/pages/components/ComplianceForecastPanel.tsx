...
export default function ComplianceForecastPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const items = Array.isArray(posJson?.items)
        ? posJson.items
      ...
      const mktItems: MarketsItem[] = Array.isArray(mktJson?.items)
        ? mktJson.items
        : Array.isArray(mktJson)
          ? mktJson
          : [];
      ...
      try {
        if (cfgRes.ok) {
          ...
          if (typeof fromCfg === "number" && fromCfg > 0 && fromCfg < 1.0)
            cap = fromCfg;
        }
      } catch {}
      setBaseCap(cap);
  ...
  const rows = useMemo<ForecastRow[]>(() => {
    ...
    const totalExposure = positions.reduce((sum, p) => {
      const price =
        typeof p.current_price === "number" ? p.current_price : p.entry_price;
  ...
  const grouped = useMemo(() => {
    const map = new Map<
      string,
      {
        ...
        exposure: number;
        alloc: number;
        breachProbBase: number;
        breachProbPolicy: number;
      }
    >();
    ...
    for (const r of rows) {
      ...
      map.set(k, {
        ...
        breachProbPolicy:
          prev.breachProbPolicy * wPrev + r.breachProbPolicy * wNew,
      });
    }
  ...
  const topDrivers = useMemo(() => {
    return [...rows]
      .map((r) => ({
        key: r.label,
        venue: r.venue,
        score: r.breachProbBase * r.exposure,
        prob: r.breachProbBase,
      }))
      ...
      <CardHeader className="flex items-start justify-between">
        <div>
          <CardTitle className="flex items-center gap-2">
            Compliance Forecasting <Badge variant="outline">Predictive</Badge>
          </CardTitle>
          <CardDescription>
            Predict future rule-breach likelihood under current exposure.
            Visualize risk buckets and simulate policy changes.
          </CardDescription>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="p-4 border rounded-lg space-y-3">
            <div className="text-sm font-medium flex items-center gap-2">
              <SlidersHorizontal className="h-4 w-4" /> Simulate Policy Change
            </div>
            <div className="text-xs text-muted-foreground">
              Lower leverage cap to reflect predicted breach reduction. Current
              cap: <strong>{(baseCap * 100).toFixed(0)}%</strong>
            </div>
              ...
              <div className="flex items-center gap-3">
                <div className="flex-1">
                  <Slider
                    ...
                    max={0.5}
                    step={0.01}
                    value={[parseFloat(capInput) || policyCap]}
          ...
          <div className="p-4 border rounded-lg space-y-3">
            <div className="text-sm font-medium flex items-center gap-2">
              <BarChart3 className="h-4 w-4" /> Settings
            </div>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                ...
                <Select
                  value={bucketBy}
                  onValueChange={(v) => setBucketBy(v as any)}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue placeholder="Bucket by" />
                  </SelectTrigger>
                  <SelectContent>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="text-sm font-medium">
              Risk Buckets ({bucketBy === "asset" ? "per-asset" : "per-venue"})
            </div>
            <Badge variant="secondary">
              Total Exposure: ${grouped.totalExposure.toFixed(2)}
            </Badge>
          </div>
          <div className="h-64">
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Bar
                    dataKey="Base"
                    fill="#ef4444"
                    name="Breach prob (base)"
                  />
                  <Bar
                    dataKey="Policy"
                    fill="#10b981"
                    name="Breach prob (policy)"
                  />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              ...
              topDrivers.map((d) => (
                <div key={d.key} className="p-3 border rounded-md">
                  ...
                  <div className="text-xs text-muted-foreground mt-1">
                    Contribution score: {d.score.toFixed(2)}
                  </div>
                  <div className="text-xs">
                    Breach probability:{" "}
                    <span className="font-mono">{fmtPct(d.prob)}</span>
                  </div>
                </div>
              ))
            ) : (
