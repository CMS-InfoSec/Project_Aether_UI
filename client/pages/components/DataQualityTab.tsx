...
import { AlertTriangle, CheckCircle2, Eye, EyeOff, Link as LinkIcon, RefreshCw, ShieldAlert, ShieldCheck, TriangleAlert, X } from "lucide-react";
...
export default function DataQualityTab() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        const arr: any[] = Array.isArray(j?.data) ? j.data : (Array.isArray(j) ? j : (Array.isArray((j as any).items) ? (j as any).items : []));
        const items: Anomaly[] = arr.map((a: any) => ({
          id: String(a.id ?? `${a.symbol || a.venue || 'item'}_${a.type || 'anomaly'}_${a.detected_at || Date.now()}`),
          symbol: String(a.symbol || "UNKNOWN"),
          type: String(a.type || a.kind || "anomaly"),
          severity: String(a.severity || a.level || "medium").toLowerCase(),
          detected_at: String(a.detected_at || a.timestamp || new Date().toISOString()),
          auto_mitigation: (a.auto_mitigation || a.mitigation || "none").toString().replace("-", "_") as any,
  ...
  const filtered = useMemo(() => {
    ...
    arr.sort((a, b) => (SEVERITY_ORDER[b.severity] || 0) - (SEVERITY_ORDER[a.severity] || 0) || (new Date(b.detected_at).getTime() - new Date(a.detected_at).getTime()));
    return arr;
  }, [anomalies, filterType, filterSeverity, showSuppressed, suppressMap]);

  const severe = useMemo(() => filtered.filter(a => a.severity === "critical" || a.severity === "high").filter(a => !ackedMap[a.id] && !suppressMap[a.id]), [filtered, ackedMap, suppressMap]);

  const acknowledge = async (a: Anomaly) => {
    setAckedMap((prev: any) => ({ ...prev, [a.id]: { at: Date.now() } }));
    try {
      await apiFetch("/api/notifications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        title: `Anomaly acknowledged: ${a.symbol}`,
        message: `${titleForType(a.type)} (${a.severity}) acknowledged by admin.`,
        severity: "info",
        category: "system",
        metadata: { anomaly_id: a.id, type: a.type, severity: a.severity, symbol: a.symbol }
      }) });
    } catch {}
  };
  ...
  const suppress = async (a: Anomaly) => {
    ...
    try {
      await apiFetch("/api/notifications", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        ...
        severity: "warning",
        category: "system",
        metadata: { anomaly_id: a.id, type: a.type, severity: a.severity, symbol: a.symbol, action: "suppress" }
      }) });
    } catch {}
  };

  const unsuppress = (a: Anomaly) => {
    setSuppressMap((prev: any) => { const next = { ...prev }; delete next[a.id]; return next; });
  };
  ...
  const stickyIcon = (sev: string) => sev === "critical" ? <ShieldAlert className="h-4 w-4"/> : <TriangleAlert className="h-4 w-4"/>;
      ...
      {severe.length > 0 && (
        <div className="sticky top-0 z-10 space-y-2">
          {severe.slice(0, 3).map(a => (
            <div key={a.id} className="p-3 rounded-md border bg-red-50 border-red-200 flex items-center justify-between">
              <div className="flex items-center gap-2 text-red-700">
                {stickyIcon(a.severity)}
                <span className="font-medium">{titleForType(a.type)} detected</span>
                <span>•</span>
                <span className="font-mono">{a.symbol}</span>
                <Badge variant="destructive" className="ml-2 capitalize">{a.severity}</Badge>
                {a.auto_mitigation && a.auto_mitigation !== "none" && (
                  <Badge variant="outline" className="ml-2">{String(a.auto_mitigation).replace("_","-")}</Badge>
                )}
              </div>
              <div className="flex items-center gap-2">
                <Button size="sm" variant="ghost" onClick={() => openRaw(a)}><Eye className="h-3 w-3 mr-1"/> View</Button>
                <Button size="sm" variant="ghost" onClick={() => acknowledge(a)}><CheckCircle2 className="h-3 w-3 mr-1"/> Acknowledge</Button>
                <Button size="sm" variant="ghost" onClick={() => suppress(a)}><EyeOff className="h-3 w-3 mr-1"/> Suppress</Button>
              </div>
            </div>
          ))}
        </div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">Data Quality <Badge variant="outline">Anomalies</Badge></CardTitle>
              <CardDescription>Stale ticks, outliers, and volume spikes. Includes severity and auto-mitigation status.</CardDescription>
            </div>
            <div className="flex items-center gap-2">
        ...
        <CardContent>
          ...
          <div className="flex items-center gap-4 mb-4 pb-4 border-b">
            ...
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="show-suppressed" className="text-sm">Show suppressed</label>
            </div>
            ...
            <div className="space-y-3">
              {filtered.map((a) => (
                <div key={a.id} className="p-3 border rounded-md bg-card">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Badge variant={severityBadgeVariant(a.severity)} className="capitalize">{a.severity}</Badge>
                      <div className="font-medium">{titleForType(a.type)}</div>
                      <div className="text-xs text-muted-foreground">{new Date(a.detected_at).toLocaleString()}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      {a.auto_mitigation && a.auto_mitigation !== "none" ? (
                        <Badge variant="outline" className="capitalize inline-flex items-center gap-1"><ShieldCheck className="h-3 w-3"/> {String(a.auto_mitigation).replace("_","-")}</Badge>
                      ) : (
                        <Badge variant="outline" className="inline-flex items-center gap-1"><ShieldAlert className="h-3 w-3"/> no auto-mitigation</Badge>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    {a.sample_url ? (
                      <a href={a.sample_url} target="_blank" rel="noreferrer" className="inline-flex items-center text-sm text-primary hover:underline">
                    ...
                    {suppressMap[a.id] ? (
                      <Button size="sm" variant="ghost" onClick={() => unsuppress(a)}>
      ...
      <Dialog open={!!showRaw} onOpenChange={(o) => !o && setShowRaw(null)}>
        <DialogContent className="max-w-3xl">
          ...
          <div className="max-h-[60vh] overflow-auto bg-muted/50 rounded p-3 text-xs">
            {showRaw?.samples ? (
              <pre className="whitespace-pre-wrap">{JSON.stringify(showRaw.samples, null, 2)}</pre>
            ) : (
              <div className="text-muted-foreground">No embedded samples. {showRaw?.sample_url ? "Use external link." : ""}</div>
            )}
          </div>
        </DialogContent>
      </Dialog>
