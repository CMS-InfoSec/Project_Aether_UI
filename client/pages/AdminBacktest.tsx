...
export default function AdminBacktest() {
  ...
  const [history, setHistory] = useState<string[]>(() => {
    try {
      return JSON.parse(localStorage.getItem("backtest_history") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const validateAndLaunch = async () => {
    ...
    try {
      ...
      if (!Array.isArray(prices) || prices.some((v) => !isFinite(Number(v))))
        throw new Error("prices must be array of finite numbers");
    } catch (e: any) {
      setLaunchError(e.message || "Invalid prices");
      return;
    }
    try {
      actions = JSON.parse(actionsJson || "[]");
      if (!Array.isArray(actions) || actions.some((v) => !isFinite(Number(v))))
        throw new Error("actions must be array of finite numbers");
    } catch (e: any) {
      setLaunchError(e.message || "Invalid actions");
      return;
    }
    if (volumesJson && volumesJson.trim().length > 0) {
      try {
        volumes = JSON.parse(volumesJson);
        if (
          !Array.isArray(volumes) ||
          volumes.some((v) => !isFinite(Number(v)))
        )
          throw new Error("volumes must be array of finite numbers");
      } catch (e: any) {
        setLaunchError(e.message || "Invalid volumes");
        return;
      }
    }
    if (prices.length === 0 || actions.length === 0) {
      setLaunchError("prices and actions are required");
      return;
    }
    if (prices.length !== actions.length) {
      setLaunchError("prices and actions must have matching lengths");
      return;
    }
    if (volumes && volumes.length !== prices.length) {
      setLaunchError("volumes length must match prices");
      return;
    }
    const seedNum = seed ? Number(seed) : undefined;
    if (seed && !Number.isFinite(seedNum!)) {
      setLaunchError("seed must be a finite number");
      return;
    }
    const balNum = Number(startingBalance);
    if (!Number.isFinite(balNum) || balNum <= 0) {
      setLaunchError("starting balance must be > 0");
      return;
    }
    const feeNum = Number(feeRate);
    if (!Number.isFinite(feeNum) || feeNum < 0 || feeNum > 1) {
      setLaunchError("fee must be in [0,1]");
      return;
    }
    ...
    try {
      const body = {
        config: {
          ...
          volumes,
          seed: seedNum,
          starting_balance: balNum,
          fee: feeNum,
        },
      };
      const r = await apiFetch("/api/strategies/backtest", {
        method: "POST",
      ...
      if (r.status === 422) {
        setLaunchError(j.error || "Validation failed");
        return;
      }
      if (r.status === 504) {
        setLaunchError("Backtest timed out");
        return;
      }
      if (!r.ok) {
        setLaunchError(j.error || "Failed to start backtest");
        return;
      }
      ...
      if (j.report_path) {
        setHistory((prev) => {
          const next = [
            j.report_path,
            ...prev.filter((p) => p !== j.report_path),
          ].slice(0, 20);
          localStorage.setItem("backtest_history", JSON.stringify(next));
          return next;
        });
  ...
  if (!report) {
    return (
      <div className="space-y-6">
        <Alert>
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>No backtest report available</AlertDescription>
        </Alert>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            ...
            Period:{" "}
            {new Date(report.test_period.start_date).toLocaleDateString()} -{" "}
            {new Date(report.test_period.end_date).toLocaleDateString()}
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Submit price/action series, optional volumes, seed, balance, and fee
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            <div>
              ...
              <textarea
                className="w-full h-24 font-mono text-xs p-2 border rounded"
                value={pricesJson}
                onChange={(e) => setPricesJson(e.target.value)}
                placeholder="[10000,10020,10035,...]"
              />
            </div>
            <div>
              <div className="text-xs font-medium mb-1">
                Actions (JSON array, -1/0/1)
              </div>
              <textarea
                className="w-full h-24 font-mono text-xs p-2 border rounded"
                value={actionsJson}
            ...
            <div>
              <div className="text-xs font-medium mb-1">
                Volumes (optional JSON array)
              </div>
              <textarea
                className="w-full h-20 font-mono text-xs p-2 border rounded"
                value={volumesJson}
            ...
            <div className="grid grid-cols-2 gap-2 items-end">
              ...
              <div className="flex items-end">
                <Button onClick={validateAndLaunch} disabled={launching}>
                  {launching ? (
                    <>
          ...
          <div className="grid md:grid-cols-3 gap-3 pt-2 border-t">
            ...
            <div className="space-x-2 flex items-end">
              ...
              <Button
                variant="outline"
                onClick={() => downloadArtifact("trades")}
              >
                Trades CSV
              </Button>
              <Button
                variant="outline"
                onClick={() => downloadArtifact("daily")}
              >
                Daily CSV
              </Button>
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Load two report JSON URLs for side-by-side comparison
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid md:grid-cols-2 gap-3">
            <Input
              placeholder="Report A URL (JSON)"
              value={compareA}
              onChange={(e) => setCompareA(e.target.value)}
            />
            <Input
              placeholder="Report B URL (JSON)"
              value={compareB}
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={async () => {
                try {
                  const r = await fetch(compareA);
                  const j = await r.json();
                  setCmpA(j as BacktestReport);
                } catch {
                  toast({
                    title: "Error",
                    description: "Failed to load A",
                    variant: "destructive",
                  });
                }
                try {
                  const r = await fetch(compareB);
                  const j = await r.json();
                  setCmpB(j as BacktestReport);
                } catch {
                  toast({
                    title: "Error",
                    description: "Failed to load B",
                    variant: "destructive",
                  });
                }
              }}
          ...
          {cmpA && cmpB && (
            <div className="grid md:grid-cols-2 gap-4">
              {[
                { label: "Net PnL", key: "net_pnl" },
                { label: "Return %", key: "return_percentage" },
                { label: "Win Rate", key: "win_rate" },
                { label: "Sharpe", key: "sharpe_ratio" },
                { label: "Max DD", key: "max_drawdown" },
              ].map((m) => (
                <div key={m.key} className="p-3 border rounded">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    <div>
                      A:{" "}
                      {typeof (cmpA!.summary as any)[m.key] === "number"
                        ? m.key === "net_pnl"
                          ? formatCurrency((cmpA!.summary as any)[m.key])
                          : formatNumber((cmpA!.summary as any)[m.key])
                        : "—"}
                    </div>
                    <div>
                      B:{" "}
                      {typeof (cmpB!.summary as any)[m.key] === "number"
                        ? m.key === "net_pnl"
                          ? formatCurrency((cmpB!.summary as any)[m.key])
                          : formatNumber((cmpB!.summary as any)[m.key])
                        : "—"}
                    </div>
                  </div>
                </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <div className="flex items-center space-x-1">
              <span
                className={`text-2xl font-bold ${getValueColor(report.summary.net_pnl)}`}
              >
              ...
              <Button
                ...
                onClick={() =>
                  copyToClipboard(report.summary.net_pnl.toString())
                }
              >
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <div className="flex items-center space-x-1">
              <span
                className={`text-2xl font-bold ${getValueColor(report.summary.return_percentage)}`}
              >
          ...
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <div className="flex items-center space-x-1">
              ...
              <Button
                ...
                className="h-6 w-6 p-0"
                onClick={() =>
                  copyToClipboard(formatPercentage(report.summary.win_rate))
                }
              >
                <Copy className="h-3 w-3" />
              </Button>
          ...
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <div className="flex items-center space-x-1">
              ...
              <Button
                ...
                className="h-6 w-6 p-0"
                onClick={() =>
                  copyToClipboard(formatNumber(report.summary.sharpe_ratio))
                }
              >
                <Copy className="h-3 w-3" />
              </Button>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <div className="flex items-center space-x-1">
              <span
                className={`text-2xl font-bold ${getValueColor(report.summary.max_drawdown)}`}
              >
              ...
              <Button
                ...
                className="h-6 w-6 p-0"
                onClick={() =>
                  copyToClipboard(formatPercentage(report.summary.max_drawdown))
                }
              >
                <Copy className="h-3 w-3" />
              </Button>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">
                  {formatNumber(report.summary.profit_factor)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">
                ...
                <span className="font-medium">
                  {formatCurrency(report.summary.expectancy)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">
                ...
                <span className="font-medium">
                  {formatNumber(report.summary.sortino_ratio)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">
                ...
                <span className="font-medium">
                  {formatPercentage(report.summary.daily_volatility)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">
                ...
                <span className="font-medium">
                  {formatPercentage(report.summary.risk_of_ruin, 4)}
                </span>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">
                  {formatPercentage(report.summary.confidence_decay, 4)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">
                ...
                <span className="font-medium">
                  {formatPercentage(
                    report.summary.alternative_scenario_success,
                  )}
                </span>
              </div>
              <div className="flex justify-between items-center">
                ...
                <span className="font-medium">
                  {formatPercentage(report.summary.streak_consistency)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">Fee %</span>
                <span className="font-medium">
                  {formatPercentage(report.summary.fee_percentage, 4)}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-muted-foreground">
                ...
                <span className="font-medium">
                  {formatNumber(report.summary.slippage_bps, 1)} bps
                </span>
              </div>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Account balance progression throughout the backtest period
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="h-80">
              ...
              <TableBody>
                {paginatedTrades.map((trade, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell
                      className={trade.pnl ? getValueColor(trade.pnl) : ""}
                    >
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-4">
              <div className="text-sm text-muted-foreground">
                ...
                {Math.min(currentPage * itemsPerPage, sortedTrades.length)} of{" "}
      ...
      {report.factor_performance && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Performance breakdown by trading factors
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              ...
              <TableBody>
                {Object.entries(report.factor_performance).map(
                  ([factor, data]) => (
                    <TableRow key={factor}>
                      ...
                      <TableCell>
                        {formatPercentage(data.return_contribution)}
                      </TableCell>
                    </TableRow>
                  ),
                )}
      ...
      {report.hypothetical_trades && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Analysis of hypothetical trading scenarios
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-2">
              <Card>
                ...
                <CardContent>
                  <div
                    className={`text-2xl font-bold ${getValueColor(report.hypothetical_trades.hypothetical_pnl)}`}
                  >
                    {formatCurrency(
                      report.hypothetical_trades.hypothetical_pnl,
                    )}
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold text-blue-600">
                    {formatPercentage(
                      report.hypothetical_trades.hypothetical_success_rate,
                    )}
                  </div>
                  <p className="text-xs text-muted-foreground mt-1">
