...
export default function AdminPortfolio() {
  ...
  const [sortField, setSortField] =
    useState<keyof PortfolioRecord>("total_balance");
  ...
  useEffect(() => {
    ...
    if (urlLimit)
      setLimit(
        Math.min(parseInt(urlLimit) || DEFAULT_PAGE_LIMIT, MAX_PAGE_LIMIT),
      );
      ...
      if (type === "prices") {
        for (const [symbol, price] of Object.entries(parsed)) {
          if (typeof price !== "number" || price <= 0) {
            throw new Error(
              `Invalid price for ${symbol}: must be a positive number`,
            );
          }
        }
      } else {
        for (const [symbol, returns] of Object.entries(parsed)) {
          ...
          for (const returnValue of returns) {
            if (typeof returnValue !== "number") {
              throw new Error(
                `Invalid return value for ${symbol}: all values must be numbers`,
              );
            }
          }
        }
  ...
  const handleRebalance = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          error instanceof Error ? error.message : "Failed to start rebalance",
        variant: "destructive",
      });
    } finally {
    ...
    return (
      <div className="space-y-4">
        <Alert>
          ...
          <AlertDescription>
            Admin access required. You do not have permission to view portfolio
            data. Please navigate back to your dashboard.
          </AlertDescription>
        </Alert>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Aggregated view of all user portfolios with system-wide rebalancing
            controls
          </p>
        </div>
          ...
          <AlertDescription>
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-muted-foreground">
                Showing {offset + 1}-
                {Math.min(offset + limit, portfolioData.total)} of{" "}
                {portfolioData.total}
              </span>
            </div>
          </AlertDescription>
      ...
      {stats && (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {formatCurrency(stats.totalUsdtBalance)}
              </div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {formatCurrency(stats.totalHedgedBalance)}
              </div>
              <p className="text-xs text-muted-foreground">
                Protected positions
              </p>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {stats.modeDistribution.live} live,{" "}
                {stats.modeDistribution.demo} demo
              </p>
            </CardContent>
          </Card>
        </div>
          ...
          <Card>
            <CardHeader className="flex items-start justify-between">
              <div>
                ...
                <CardDescription>
                  Click column headers to sort. Use pagination controls to
                  navigate.
                </CardDescription>
              </div>
                        ...
                        {portfolioData.items.length > 0 ? (
                          portfolioData.items.map((portfolio) => (
                            ...
                            >
                              ...
                              <TableCell>
                                {formatCurrency(portfolio.hedged_balance)}
                              </TableCell>
                              ...
                              <TableCell className="text-sm">
                                {new Date(
                                  new Date(portfolio.last_updated).getTime() +
                                    7 * 24 * 60 * 60 * 1000,
                                ).toLocaleString()}
                              </TableCell>
                              <TableCell className="text-right space-x-2">
                                <a
                                  href={`/audit?query=user_id:${encodeURIComponent(portfolio.user_id)}`}
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    setRowError(null);
                                    setRowAck(false);
                                    setRowPricesJson(pricesJson);
                                    setRowReturnsJson(returnsJson);
                                  }}
                                >
                                  Details
                                </Button>
          ...
          <Card>
            <CardHeader className="flex items-start justify-between">
              <div>
                ...
                <CardDescription>
                  Trigger system-wide portfolio rebalancing with current market
                  data
                </CardDescription>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <HelpTip content="JSON object mapping SYMBOL/USDT to current price, e.g. {'BTC/USDT': 43500}. Used to price portfolios." />
                </div>
                <Textarea
                  id="pricesJson"
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <HelpTip content="JSON object mapping SYMBOL/USDT to an array of recent returns (fractions), e.g. {'BTC/USDT': [0.02, -0.01]}. Used for risk and allocation." />
                </div>
                <Textarea
                  id="returnsJson"
                  ...
                  onChange={(e) => {
                    ...
                    if (jsonErrors.returns)
                      validateJson(e.target.value, "returns");
                  }}
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>
                      Confirm System-Wide Rebalance
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                      This will trigger a rebalance across all active portfolios
                      using the provided prices and returns data. This action
                      cannot be undone.
                    </AlertDialogDescription>
                    {stats && (
                      <div className="mt-4 p-3 bg-muted rounded-lg">
                        <div className="text-sm space-y-1">
                          <div>
                            <strong>Active portfolios:</strong>{" "}
                            {stats.modeDistribution.live +
                              stats.modeDistribution.demo}
                          </div>
                          <div>
                  ...
                  <div className="px-6 -mt-4 mb-2">
                    <div className="flex items-center gap-2 p-2 border rounded-md">
                      <input
                        ...
                        type="checkbox"
                        checked={ackGlobal}
                        onChange={(e) => setAckGlobal(e.target.checked)}
                      />
                      <Label htmlFor="ackGlobal" className="text-xs">
                        I understand this will trigger a bulk rebalance across
                        active portfolios.
                      </Label>
                  ...
                  <AlertDialogFooter>
                    <AlertDialogCancel onClick={() => setAckGlobal(false)}>
                      Cancel
                    </AlertDialogCancel>
                    <AlertDialogAction
                      onClick={handleRebalance}
                      disabled={!ackGlobal || isRebalancing}
                    >
                      Confirm Rebalance
                    </AlertDialogAction>
                  </AlertDialogFooter>
                  ...
                  rebalanceHistory.slice(0, 5).map((event) => (
                    <div key={event.id} className="border rounded-lg p-3">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                        ...
                        <div>
                          Value: {formatCurrency(event.totalValueRebalanced)}
                        </div>
      ...
      {rebalanceHistory.length > 0 && (
        <div className="mt-2 p-3 border rounded-md text-sm bg-muted/50">
          <div className="flex items-center justify-between">
            <div>
              Last global rebalance:{" "}
              {new Date(rebalanceHistory[0].timestamp).toLocaleString()} •{" "}
              {rebalanceHistory[0].reason}
            </div>
            <a
              href={`/audit?ref=${rebalanceHistory[0].id}`}
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <HelpTip content="Override prices for this user only. Same format as global prices JSON." />
              </div>
              <Textarea
                rows={4}
                value={rowPricesJson}
                onChange={(e) => setRowPricesJson(e.target.value)}
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <HelpTip content="Override returns array for this user only. Same format as global returns JSON." />
              </div>
              <Textarea
                rows={4}
                value={rowReturnsJson}
                onChange={(e) => setRowReturnsJson(e.target.value)}
                className="font-mono text-xs"
              />
            </div>
            <div className="flex items-center gap-2 p-2 border rounded-md">
              <input
                ...
                type="checkbox"
                checked={rowAck}
                onChange={(e) => setRowAck(e.target.checked)}
              />
              <Label htmlFor="ack" className="text-xs">
                I understand this will rebalance active portfolios. Include this
                user in audit search.
              </Label>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={async () => {
                setRowError(null);
                if (!rowAck) {
                  setRowError("Please acknowledge the action");
                  return;
                }
                try {
                  ...
                  if (typeof returns !== "object" || Array.isArray(returns))
                    throw new Error("Returns must be an object");
                } catch (e: any) {
                  setRowError(e.message || "Invalid JSON");
                  return;
                }
                setRowIsSubmitting(true);
                try {
                  const r = await apiFetch("/api/admin/portfolio/rebalance", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                      prices: JSON.parse(rowPricesJson),
                      returns: JSON.parse(rowReturnsJson),
                      actor: "admin@example.com",
                  ...
                  if (!r.ok) {
                    throw new Error(j.message || j.error || "Rebalance failed");
                  }
                  toast({
                    title: "Rebalance Started",
                    description: `Queued across ${j.rebalanced} portfolios`,
                  });
                  setTimeout(async () => {
                    await Promise.all([
                      fetchPortfolios(),
