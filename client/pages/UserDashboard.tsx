...
export default function UserDashboard() {
  ...
  const [performanceData, setPerformanceData] = useState<
    Array<{ date: string; returns: number; benchmark: number }>
  >([]);
  const [allocationData, setAllocationData] = useState<
    Array<{ name: string; value: number; amount?: number }>
  >([]);
  ...
  const [regimeRange, setRegimeRange] = useState<{
    ...
    try {
      const s = localStorage.getItem("regime_range");
  ...
  const loadDailyReport = async () => {
    ...
    try {
      ...
      if (!mounted) return;
      setDailyReport({
        portfolioValue: new Intl.NumberFormat("en-US", {
          style: "currency",
          currency: "USD",
        }).format(1000000 + (data?.totalReturn || 0)),
        dailyChange: new Intl.NumberFormat("en-US", {
          style: "currency",
          currency: "USD",
        }).format(data?.totalReturn || 0),
      ...
      const dr = Array.isArray(data?.dailyReturnsData)
  ...
  const loadWeeklyReport = async () => {
    ...
    try {
      ...
      const data = j?.data || j;
      if (!mounted) return;
      const sortedBest = Array.isArray(data?.weeklyAssetData)
        ? [...data.weeklyAssetData].sort(
            (a: any, b: any) => (b.returns || 0) - (a.returns || 0),
          )
        : [];
      const sortedWorst = Array.isArray(data?.weeklyAssetData)
        ? [...data.weeklyAssetData].sort(
            (a: any, b: any) => (a.returns || 0) - (b.returns || 0),
          )
        : [];
      setWeeklyReport({
        weeklyReturn: new Intl.NumberFormat("en-US", {
          style: "currency",
          currency: "USD",
        }).format(data?.weeklyReturn || 0),
        ...
        sharpeRatio: Number(data?.sharpeRatio ?? 0).toFixed
    ...
    try {
      ...
      setAllocationData(
        assets.map((a: any) => ({
          ...
          amount: Number(a.currentPrice) || undefined,
        })),
      );
    } catch (e) {
  ...
  const loadPortfolioPanel = async () => {
    ...
    try {
      ...
      const eff =
        Number(data?.effectivePercent ?? data?.hedgePercent ?? 0) || 0;
      setHedgePercent(eff);
    } catch (e) {}
  };
      ...
      try {
        ...
        for (const c of items) {
          ...
          if (
            sev.includes("fail") ||
            sev.includes("error") ||
            sev.includes("violation")
          ) {
            aggregated.push({
              id: String(c.id || `${Date.now()}_${Math.random()}`),
              timestamp: new Date(c.timestamp || Date.now()).getTime(),
              title: `Compliance: ${c.rule || c.rule_type || "Violation"}`,
              message:
                c.message ||
                `Trade ${c.tradeId || c.trade_id || ""} by ${c.user || c.actor || ""}`,
              severity: "error",
              read: false,
              source: "compliance",
            });
      ...
      try {
        ...
        for (const it of items) {
          aggregated.push({
            ...
            title: `Audit: ${it.action}`,
            message: it.details || `Actor ${it.actor}`,
            severity: it.success === false ? "warning" : "info",
            read: false,
            source: "audit",
          });
        }
  ...
  const loadExecutionHeatmap = async () => {
    ...
    try {
      ...
      let lat: any = null;
      try {
        lat = await getJson<any>(`/api/execution/latency?${params.toString()}`);
      } catch {}
      if (!lat) {
        try {
          lat = await getJson<any>(`/execution/latency?${params.toString()}`);
        } catch {}
      }
      const litems: any[] = Array.isArray(lat?.data)
        ? lat.data
        : Array.isArray(lat)
          ? lat
          : [];
      ...
      for (const ep of impactEndpoints) {
        try {
          imp = await getJson<any>(ep);
          if (imp) break;
        } catch {}
      }
      const iitems: any[] = Array.isArray(imp?.data)
        ? imp.data
        : Array.isArray(imp)
          ? imp
          : [];
      ...
      for (const it of litems) {
        ...
        buckets.add(b);
        const p50Lat =
          Number(it.p50_latency_ms ?? it.p50 ?? it.lat_p50 ?? 0) || 0;
        const p95Lat =
          Number(it.p95_latency_ms ?? it.p95 ?? it.lat_p95 ?? 0) || 0;
        const p50Slip =
          Number(it.p50_slippage_bps ?? it.slip_p50 ?? it.p50_slip ?? 0) || 0;
        const p95Slip =
          Number(it.p95_slippage_bps ?? it.slip_p95 ?? it.p95_slip ?? 0) || 0;
      ...
      const merged: Record<
        string,
        { venue: string; byBucket: Record<string, any> }
      > = {};
      for (const it of iitems) {
        ...
        const discrepant =
          predicted !== 0
            ? Math.abs(realized - predicted) /
                Math.max(1e-9, Math.abs(predicted)) >
              0.25
            : Math.abs(realized) > 0;
      ...
      let bucketList = Array.from(buckets).sort((a, b) =>
      ...
      if (regimeRange) {
        const inRange = (b: string) => {
          ...
          if (Number.isFinite(t))
            return t >= regimeRange.from && t <= regimeRange.to;
          return true;
        };
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="overview" className="space-y-6">
        ...
        <TabsContent value="overview" className="space-y-6">
          <RegimeTimelineRibbon
            onSelect={(r) =>
              setRegimeRange(r ? { from: r.from, to: r.to } : null)
            }
            selected={regimeRange || undefined}
          />
          {/* Key Metrics Cards */}
          ...
          <Card>
            <CardHeader className="flex items-start justify-between">
              <div>
                ...
                <CardDescription>
                  Holdings, allocations, hedge exposure, and unrealized P&L
                </CardDescription>
              </div>
            ...
            <CardContent>
              <div className="space-y-3">
                <div>
                  ...
                  <div className="w-full h-3 bg-muted rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, Math.max(0, hedgePercent * 100)).toFixed(0)}%`,
                      }}
                    />
                  </div>
                  <div className="text-xs mt-1">
                    {(hedgePercent * 100).toFixed(0)}% hedged
                  </div>
                </div>
                    ...
                    <div className="space-y-2">
                      {portfolioHoldings.map((h) => (
                        ...
                        >
                          <div>{h.symbol}</div>
                          <div
                            className={`${h.pnl >= 0 ? "text-accent" : "text-destructive"} font-medium`}
                          >
                            {h.pnl >= 0 ? "+" : ""}
                      ...
                      <div className="border-t pt-2 flex items-center justify-between text-sm">
                        ...
                        <div
                          className={`${portfolioPnL >= 0 ? "text-accent" : "text-destructive"} font-semibold`}
                        >
                          {portfolioPnL >= 0 ? "+" : ""}
                ...
                <div className="text-xs text-muted-foreground">
                  Portfolio Market Value: {formatCurrency(portfolioTotal)}
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex items-start justify-between">
              <div>
                ...
                <CardDescription>
                  Venue health, depth, spreads, and arbitrage
                </CardDescription>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={loadCrossMarket}
                  disabled={crossLoading}
                >
                  <RefreshCw
                    className={`h-4 w-4 ${crossLoading ? "animate-spin" : ""}`}
                  />
                </Button>
              </div>
            </CardHeader>
                      ...
                      <div key={v.name + idx} className="p-2 border rounded-md">
                        ...
                        <div className="w-full h-2 bg-muted rounded-full overflow-hidden mt-2">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, v.depthUsd ? (Math.log10(v.depthUsd + 1) / 6) * 100 : 0)}%`,
                            }}
                          />
                        </div>
                      </div>
                      ...
                      >
                        <div className="flex items-center justify-between text-sm">
                          ...
                          <div className="text-green-600 font-semibold">
                            {a.spreadPct.toFixed(2)}%
                          </div>
                        </div>
                        <div className="text-xs text-muted-foreground">
                          Buy on {a.buyVenue} → Sell on {a.sellVenue}
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <Card>
            <CardHeader className="flex items-start justify-between">
              <div>
                ...
                <CardDescription>
                  Venue × time buckets; slippage (bps), latency (P50/P95), and
                  fill rates
                </CardDescription>
              </div>
              <div className="flex items-center gap-2">
            ...
            <CardContent className="space-y-3">
              <div className="grid md:grid-cols-4 gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    <Label>Venue</Label>
                    <HelpTip content="Filter by venue or All." />
                  </div>
                  <Select value={execVenue} onValueChange={setExecVenue}>
                    ...
                    <SelectContent>
                      ...
                      {Array.from(new Set(execRows.map((r) => r.venue))).map(
                        (v) => (
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <Label>Symbol</Label>
                    <HelpTip content="Optional symbol filter (e.g., BTC/USDT)." />
                  </div>
                  <Input
                    value={execSymbol}
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <Label>Window</Label>
                    <HelpTip content="Aggregation window." />
                  </div>
                  <Select
                    value={execWindow}
                    onValueChange={(v) => {
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    type="checkbox"
                    checked={execAuto}
                    onChange={(e) => setExecAuto(e.target.checked)}
                  />
                </div>
              </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b">
                      ...
                      {execBuckets.map((b) => (
                        <th key={b} className="text-center p-2">
                          {b}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {execRows
                      .filter((r) =>
                        execVenue === "all" ? true : r.venue === execVenue,
                      )
                      .map((r) => (
                        <tr key={r.venue} className="border-b">
                          ...
                          {execBuckets.map((b) => {
                            ...
                            return (
                              ...
                              >
                                <div
                                  className={`rounded-md p-2 ${c?.discrepant ? "ring-1 ring-destructive" : ""}`}
                                  style={{ background: bg }}
                                  title={title}
                                >
                                  <div className="flex items-center justify-between">
                                  ...
                                  <div className="text-[11px]">
                                    lat P50/P95:{" "}
                                    {c
                                      ? `${Math.round(c.p50Lat || 0)}/${Math.round(c.p95Lat || 0)} ms`
                                      : "—"}
                                  </div>
                                  <div className="text-[11px]">
                                    fill:{" "}
                                    {c
                                      ? `${Math.round((c.fill || 0) * 100)}%`
                                      : "—"}
                                  </div>
                                </div>
                              </td>
