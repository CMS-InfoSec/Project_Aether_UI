...
export default function Observability() {
  ...
  const pollReadiness = useCallback(async () => {
    try {
      ...
      window.dispatchEvent(new CustomEvent('aether:readiness', { detail: { ready: !!j.ready } }));
  ...
  const pollDependencies = useCallback(async () => {
    ...
    try {
      ...
      setDeps(list);
      setDepsErr(null);
    } catch (e: any) { setDepsErr(e.message || 'Failed'); }
  }, [isPrivileged]);
  ...
  const pollLiveness = useCallback(async () => {
    ...
    try {
      ...
      setLiveDetails(j);
    } catch (e: any) { setLiveErr(e.message || 'Failed'); }
  }, [isPrivileged]);
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      ...
      if (r.status === 401 || r.status === 403) { setMetricsVisible(false); return; }
      if (!r.ok) throw new Error(`${r.status} ${r.statusText}`);
      const txt = await r.text();
      setMetrics(txt);
      setMetricsErr(null);
    } catch (e: any) { setMetricsErr(e.message || 'Failed'); }
  }, [isPrivileged]);

  // Initial loads
  useEffect(() => { pollReadiness(); }, [pollReadiness]);
  useEffect(() => {
    const a = setInterval(pollReadiness, 30000);
    return () => clearInterval(a);
  }, [pollReadiness]);

  useEffect(() => { pollDependencies(); }, [pollDependencies]);
  useEffect(() => {
    if (!isPrivileged) return;
  ...
  useEffect(() => { pollLiveness(); }, [pollLiveness]);
  useEffect(() => {
    if (!isPrivileged) return;
  ...
  useEffect(() => { if (metricsAuto) { fetchMetrics(); const t = setInterval(fetchMetrics, 30000); return () => clearInterval(t); } }, [metricsAuto, fetchMetrics]);

  // Cooldowns
  useEffect(() => { if (cooldownUser>0) { const t = setInterval(()=> setCooldownUser(v=> Math.max(0,v-1)), 1000); return ()=> clearInterval(t); } }, [cooldownUser]);
  useEffect(() => { if (cooldownGlobal>0) { const t = setInterval(()=> setCooldownGlobal(v=> Math.max(0,v-1)), 1000); return ()=> clearInterval(t); } }, [cooldownGlobal]);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      attempt += 1;
      try {
        const r = await fetch(`/api/tasks/${encodeURIComponent(taskId)}`, { cache: 'no-cache' });
        if (!r.ok) throw new Error(`${r.status} ${r.statusText}`);
        const j = await r.json();
        setTaskStatus(j);
        if (j.status === 'SUCCESS' || j.status === 'FAILED' || j.status === 'CANCELLED') return; // stop
      } catch (e) { /* keep polling with backoff */ }
    ...
    <div className="space-y-6">
      ...
      {(ready === false || readyErr) && (
        <Alert variant="destructive"><AlertDescription>
          {!ready ? 'System not ready. Investigate dependencies below.' : readyErr}
          ...
          {ready === null ? (
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {serviceCards.map((s: any) => (
                <Badge key={s.name} variant={s.ok ? 'outline' : 'destructive'} className={`${s.required? '' : 'opacity-80'}`}>
                  {s.name}: {s.ok ? 'ok' : (s.skipped? 'skipped':'error')}{stale(s.checked_at)? ' • stale':''}
                </Badge>
              ))}
            </div>
          )}
          ...
          <CardContent>
            ...
            <div className="overflow-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {deps.map((d) => (
                    <tr key={d.id} className={`border-t ${d.ok? '' : 'bg-destructive/5'}`}>
                      <td className="p-2 font-medium capitalize">{d.id}</td>
                      <td className="p-2">
                        <Badge variant={d.ok? 'outline' : (d.skipped? 'secondary':'destructive')}>{d.ok? 'ok' : (d.skipped? 'skipped' : 'error')}</Badge>
                        {d.error && <div className="text-xs text-muted-foreground">{d.error}</div>}
                      </td>
                      <td className="p-2">{d.code ?? ''}</td>
                      <td className="p-2 whitespace-nowrap">{new Date(d.checked_at).toLocaleString()} {stale(d.checked_at) && <Badge variant="destructive" className="ml-2">stale</Badge>}</td>
                      <td className="p-2">{d.timeout ?? ''}</td>
                      <td className="p-2">
                        <div className="flex items-center gap-2">
                          <code className="text-xs">{d.request_id}</code>
                          {d.request_id && <Button size="icon" variant="ghost" onClick={()=> copyText(d.request_id!)}><Copy className="h-4 w-4"/></Button>}
                        </div>
                      </td>
                      <td className="p-2">
                        <div className="flex items-center gap-2">
                          <Button size="sm" variant="outline" onClick={()=> setSelectedDep(d)}>Diagnostics</Button>
                          <Button size="sm" onClick={pollDependencies}>Retry</Button>
                        </div>
                      </td>
                    </tr>
                  ))}
                  {deps.length === 0 && (
                    <tr><td className="p-2 text-muted-foreground" colSpan={7}>No dependency data yet</td></tr>
                  )}
                </tbody>
              </table>
            </div>
            ...
            {selectedDep && (
              <div className="mt-4 p-3 border rounded bg-muted/30">
                <div className="flex items-center justify-between">
                  <div className="font-medium">Diagnostics: {selectedDep.id}</div>
                  <Button size="sm" variant="ghost" onClick={()=> setSelectedDep(null)}>Close</Button>
                </div>
                <div className="grid md:grid-cols-2 gap-2 text-sm mt-2">
                  <div>OK: {String(selectedDep.ok)}</div>
                  <div>Code: {selectedDep.code ?? '-'}</div>
                  <div>Checked: {new Date(selectedDep.checked_at).toLocaleString()}</div>
                  <div>Timeout: {selectedDep.timeout ?? '-'}</div>
                  <div>Skipped: {String(!!selectedDep.skipped)}</div>
                  <div>Request ID: <code>{selectedDep.request_id}</code></div>
                </div>
                {selectedDep.error && <div className="mt-2 text-xs">Error: {selectedDep.error}</div>}
                <div className="mt-2">
                  <Button size="sm" variant="outline" onClick={()=> copyText(JSON.stringify(selectedDep,null,2))}><Copy className="h-4 w-4 mr-2"/>Copy JSON</Button>
                </div>
              </div>
            )}
          </CardContent>
            ...
            {liveDetails ? (
              <div className="grid md:grid-cols-2 gap-3">
                {liveDetails.dependencies?.map((d:any)=> (
                  <div key={d.name} className={`p-3 border rounded ${d.ok? '':'bg-destructive/5'}`}>
                    <div className="flex items-center justify-between">
                      <div className="font-medium">{d.name}</div>
                      <Badge variant={d.ok? 'outline':'destructive'}>{d.ok? 'Healthy':'Unavailable'}</Badge>
                    </div>
          ...
          <CardContent>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" variant="outline" onClick={()=>{ const blob=new Blob([metrics],{type:'text/plain'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='metrics.txt'; a.click(); }}><Download className="h-4 w-4 mr-2"/>Download</Button>
                <Button size="sm" variant="outline" onClick={()=> copyText(metrics)}><Copy className="h-4 w-4 mr-2"/>Copy</Button>
                <Button size="sm" variant="ghost" asChild><a href="/api/metrics" target="_blank" rel="noreferrer"><ExternalLink className="h-4 w-4 mr-2"/>Open</a></Button>
              </div>
            </div>
            <pre className="max-h-64 overflow-auto text-xs p-3 bg-muted/40 rounded border whitespace-pre-wrap">{metrics || 'No metrics loaded'}</pre>
            {metricsErr && <Alert className="mt-2" variant="destructive"><AlertDescription>Paused due to error: {metricsErr}</AlertDescription></Alert>}
          </CardContent>
        </Card>
      )}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="flex flex-wrap gap-2 items-center">
              <Button disabled={cooldownUser>0} onClick={async ()=>{ try{ await fetch('/api/tasks/data-refresh', { method:'POST' }); setCooldownUser(30); }catch{} }}>User Data Refresh {cooldownUser>0 && <Badge variant="secondary" className="ml-2">{cooldownUser}s</Badge>}</Button>
              <Button disabled={cooldownGlobal>0} variant="outline" onClick={async ()=>{ try{ await fetch('/api/data/refresh', { method:'POST' }); setCooldownGlobal(60); }catch{} }}>Fleet-wide Refresh {cooldownGlobal>0 && <Badge variant="secondary" className="ml-2">{cooldownGlobal}s</Badge>}</Button>
            </div>
            <div className="grid md:grid-cols-3 gap-3 items-end">
              ...
              <div className="flex items-center gap-2">
                <Button onClick={async ()=>{
                  try{
                    const r = await fetch(`/api/data/price-series?symbol=${encodeURIComponent(priceSymbol)}`);
                    const j = await r.json();
                    const id = j.message || j.task_id || '';
                    if (id) { setTaskId(id); setTaskStatus(null); setStopPolling(false); }
                  }catch{}
                }}>Queue Job</Button>
                {taskId && <Button variant="ghost" onClick={()=> setStopPolling(true)}>Stop Polling</Button>}
              </div>
            </div>
            {taskId && (
              <div className="text-sm">Task: <code>{taskId}</code> • Status: <Badge variant={taskStatus?.status==='SUCCESS'? 'outline' : 'secondary'}>{taskStatus?.status || 'PENDING'}</Badge></div>
            )}
          </CardContent>
        </Card>
      )}
