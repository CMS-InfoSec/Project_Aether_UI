...
interface EventsResponse<T> { status: string; data: { total: number; items: T[]; next?: string } }
...
export default function AuditLogs() {
  ...
  const fetchTrades = useCallback(async (cursor?: string, append = false) => {
  ...
  const fetchBalances = useCallback(async (cursor?: string, append = false) => {
    const qs = buildQuery(cursor);
    const headers: Record<string,string> = {};
    ...
    setBalances(prev => append ? prev.concat(j.data.items) : j.data.items);
  }, [symbol, action, since, limit]);

  const load = useCallback(async () => {
    setError(null);
    try {
      await Promise.all([fetchTrades(undefined,false), fetchBalances(undefined,false)]);
  ...
  useEffect(() => {
    ...
    try {
      wt = new WebSocket(wsBase.replace(/^http/,'ws').replace(/\/$/, '') + '/api/v1/ws/trades');
      wt.onmessage = (ev) => {
        try { const msg = JSON.parse(ev.data); if (msg && msg.id) setTrades(prev => [msg as TradeEvent, ...prev].slice(0, 500)); } catch {}
      };
    } catch {}
    try {
      wb = new WebSocket(wsBase.replace(/^http/,'ws').replace(/\/$/, '') + '/api/v1/ws/balances');
      wb.onmessage = (ev) => {
        try { const msg = JSON.parse(ev.data); if (msg && msg.id) setBalances(prev => [msg as BalanceEvent, ...prev].slice(0, 500)); } catch {}
      };
    } catch {}
    return () => { try { wt?.close(); } catch {} try { wb?.close(); } catch {} };
  }, []);

  const exportJSON = async () => {
    const payload = { filters: { symbol, action, user, since, limit },
      trades: await Promise.all(trades.map(async t => ({ ...t, integrity: await sha256Hex(JSON.stringify(t)) }))),
      balances: await Promise.all(balances.map(async b => ({ ...b, integrity: await sha256Hex(JSON.stringify(b)) }))) };
  ...
  const exportCSV = async () => {
    ...
    const headers = Array.from(rows.reduce((set: Set<string>, r:any)=>{ Object.keys(r).forEach(k=> set.add(k)); return set; }, new Set<string>(['type','id','timestamp'])));
    const digests = await Promise.all(rows.map(r => sha256Hex(JSON.stringify(r))));
    const csv = [headers.concat(['integrity']).join(',')].concat(rows.map((r,idx)=> headers.concat(['integrity']).map(h=>{
      const v = (h==='integrity'? digests[idx] : (r as any)[h]); if (v===undefined||v===null) return '';
      const s = typeof v==='string'? v.replace(/"/g,'""') : String(v);
      return /[,\n"]/.test(s) ? `"${s}"` : s;
    }).join(','))).join('\n');
    const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'audit_export.csv'; a.click();
  };
    ...
    <div className="space-y-6">
              ...
              <tbody>
                {trades.map(t => (
                  <tr key={t.id} className="border-t align-top">
                    ...
                    <td className="p-2">
                      <Button size="sm" variant="outline" onClick={()=> setOpenTrade(openTrade === t.id ? null : t.id)}>
                        {openTrade === t.id ? <ChevronUp className="h-4 w-4"/> : <ChevronDown className="h-4 w-4"/>}
                      </Button>
                      {openTrade === t.id && (
                ...
                {trades.length === 0 && (
                  <tr><td className="p-2 text-muted-foreground" colSpan={10}>No trade events</td></tr>
                )}
              </tbody>
            </table>
          </div>
          <div className="mt-3 flex items-center gap-2">
            <Button variant="outline" disabled={!tNext} onClick={()=> fetchTrades(tNext, true)}>Load more</Button>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="overflow-auto">
            <table className="w-full text-sm">
              <thead><tr><th className="text-left p-2">ID</th><th className="text-left p-2">Timestamp</th><th className="text-left p-2">Account</th><th className="text-left p-2">Symbol</th><th className="text-left p-2">Delta</th><th className="text-left p-2">Reason</th><th className="text-left p-2">HMAC</th><th className="text-left p-2">Request ID</th><th className="text-left p-2">Expand</th></tr></thead>
              <tbody>
                {balances.map(b => (
                  <tr key={b.id} className="border-t align-top">
                    <td className="p-2">{b.id}</td>
                    <td className="p-2 whitespace-nowrap">{new Date(b.timestamp).toLocaleString()}</td>
                    ...
                    <td className="p-2">
                      <Button size="sm" variant="outline" onClick={()=> setOpenBalance(openBalance === b.id ? null : b.id)}>
                        {openBalance === b.id ? <ChevronUp className="h-4 w-4"/> : <ChevronDown className="h-4 w-4"/>}
                      </Button>
                      {openBalance === b.id && (
                        <div className="mt-2 p-2 border rounded bg-muted/30">
                          <div className="text-xs text-muted-foreground mb-1">Raw payload</div>
                ...
                {balances.length === 0 && (
                  <tr><td className="p-2 text-muted-foreground" colSpan={9}>No balance events</td></tr>
                )}
              </tbody>
            </table>
          </div>
          <div className="mt-3 flex items-center gap-2">
            <Button variant="outline" disabled={!bNext} onClick={()=> fetchBalances(bNext, true)}>Load more</Button>
          </div>
        </CardContent>
      </Card>
    </div>
