    ...
    try {
      ...
      const hostOk = (() => {
        try {
          ...
          return (status?.allowed_hosts || []).includes(h);
        } catch {
          return false;
        }
      })();
      if (!hostOk) {
        setError(
          `URL host not allowed. Allowed: ${(status?.allowed_hosts || []).join(", ")}`,
        );
        return;
      }
      if (
        typeof nonce !== "number" ||
        nonce <= (status?.last_nonce || 0) ||
        sentNonces.has(nonce)
      ) {
        setError("Invalid or duplicate nonce");
        return;
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold">Mobile Push Console</h1>
        ...
        <CardContent>
          {status ? (
            <div className="text-sm">
              Ready: {String(status.ready)} • Queue: {status.queue_depth} • Last
              nonce: {status.last_nonce} • Allowed hosts:{" "}
              {(status.allowed_hosts || []).join(", ")}
            </div>
          ) : (
            "Loading..."
          )}
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          <Input
            placeholder="Tokens (one per line)"
            value={token}
          ...
          <div className="flex items-center gap-2">
            <Dialog>
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Enter the shared secret to compute HMAC-SHA256 over
                    ...
                    onClick={async () => {
                      ...
                      if (!secret) {
                        toast({
                          ...
                          description: "Enter a shared secret",
                          variant: "destructive",
                        });
                        return;
                      }
            ...
            <Button variant="ghost" onClick={() => setSignature("")}>
              Clear Sig
            </Button>
          </div>

          <Button onClick={send} disabled={cooldown > 0}>
            {cooldown > 0 ? `Retry in ${cooldown}s` : "Send"}
          </Button>
        </CardContent>
      </Card>
    </div>
