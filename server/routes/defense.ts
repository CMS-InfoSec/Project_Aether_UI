...
export type DefenseEvent = {
  ...
  response: 'quarantined' | 'down_weighted' | 'circuit_breaker' | 'alerted' | 'none' | string;
  symbol?: string;
  venue?: string;
  detected_at: string;
  details?: string;
...
function sampleEvents(): DefenseEvent[] {
  ...
  const mk = (i:number, patch: Partial<DefenseEvent>): DefenseEvent => ({
    ...
    actor: 'bot',
    status: 'active',
    response: defenseSettings.autoDefenseEnabled && !defenseSettings.operatorOverride ? 'quarantined' : 'alerted',
    symbol: 'BTC/USDT',
    venue: 'binance',
  ...
  return [
    mk(1, {}),
    mk(2, { type:'shilling', severity:'medium', actor:'human', status:'observed', response:'alerted', details:'Coordinated social shilling detected' }),
    mk(3, { type:'spoofing', severity:'critical', actor:'bot', status:'active', response: defenseSettings.autoDefenseEnabled && !defenseSettings.operatorOverride ? 'circuit_breaker' : 'alerted', symbol:'ETH/USDT' }),
    mk(4, { type:'wash_trading', severity:'low', actor:'unknown', status:'mitigated', response:'down_weighted', symbol:'ALT/USDT', venue:'okx' }),
  ];
}
